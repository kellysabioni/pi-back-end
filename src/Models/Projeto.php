...
class Projeto{
    ...
    public function __construct(?int $id = null , string $nome, string $CEP, string $rua, string $numero, string $bairro, string $cidade, string $UF, string $telefone, Categoria $categoria = Categoria::Indefinido , ?int $usuarios_id = null, ?int $eventos_id = null) {
        $this-> setId ($id);
        $this-> setNome ($nome);
        $this-> setCEP ($CEP);
        $this-> setRua ($rua);
